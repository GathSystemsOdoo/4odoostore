...
class eq_ir_actions_report_xml(models.Model):
    ...
    def _get_wiz_action(self):
        ...
        if self.eq_report_wiz_action:
            ...
            rep_wiz_action_id = self.pool['ir.model.data'].xmlid_to_res_id(self._cr, self._uid, 'eq_print.eq_print_wiz_action', raise_if_not_found=True)
            rep_wiz_action_vals = act_window_obj.copy_data(self._cr, self._uid, rep_wiz_action_id, context=self.env.context)
            rep_wiz_action_vals['name'] = self.name + ' Remote Print'
            rep_wiz_action_vals['context'] = "{'report_name': '" + self.report_name + "' }"
            new_rep_wiz_action_id = act_window_obj.create(self._cr, self._uid, rep_wiz_action_vals, context=self.env.context)
    ...
    def create(self, cr, uid, vals, context = None):
        res = super(eq_ir_actions_report_xml, self).create(cr, uid, vals, context=context)
        self._update_ir_values(cr, uid, res, context)
        return res
 

    def write(self, cr, uid, ids, vals, context = None):
        res = super(eq_ir_actions_report_xml, self).write(cr, uid, ids, vals, context=context)
        self._update_ir_values(cr, uid, ids, context)
        return res
